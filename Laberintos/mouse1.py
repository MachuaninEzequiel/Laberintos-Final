...
class Mouse1:
    ...
    def move(self):
        ...
        # Heurística para `Mouse1`: Seguir la pared izquierda
    ...
    def can_move_left(self):
        # Verifica si puede moverse a la izquierda en función de la dirección actual
        if self.direction == 'E' and not self.maze[self.y][self.x].north:
            return True
        elif self.direction == 'S' and not self.maze[self.y][self.x].east:
            return True
        elif self.direction == 'W' and not self.maze[self.y][self.x].south:
            return True
        elif self.direction == 'N' and not self.maze[self.y][self.x].west:
            return True
        return False
