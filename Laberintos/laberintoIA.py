...
def save_statistics(time_taken, movements, cell_visits):
    data = {'time': time_taken, 'movements': movements, 'cell_visits': cell_visits}
    df = pd.DataFrame([data])
    df.to_csv('maze_statistics.csv', mode='a', header=not pd.io.common.file_exists('maze_statistics.csv'), index=False)

# Bucle principal
setup(Mouse)  # Se puede cambiar a Mouse2 o MouseIA según el ratón deseado
running = True
while running:
